...
from loader import load_train, load_dev, load_test, load_train_handcrafted, load_dev_handcrafted, load_hard_handcrafted, load_movies_handcrafted
...
def get_data(sequence_length):
    ...
    test_text = new_preprocessing(test['reviewText'], vocab,word_idx, idx_word, max_length=sequence_length)
...
def ablation_study(
    ...
    for i in range(num_features):
        ...
        if temp_num_features != 0:
            model = sentiNN(hidden_size1, hidden_size2, num_layers1, num_layers2, sequence_length, vocab_len, emb_dim, num_features=temp_num_features).float()
        else:
            model = sentiNN(hidden_size1, hidden_size2, num_layers1, num_layers2, sequence_length, vocab_len, emb_dim).float()
        n_model, epoch_score = training(model, train_batches, dev_batches, learning_rate, momentum, num_epoch)
        ...
        ablation_scores[temp_num_features] = (n_model, epoch_score,model)
...
def main():
    sequence_length = 60
    train_batches, dev_batches, vocab, data_shape, test_X,test_y = get_data(sequence_length)
    ...
    ablation_scores = ablation_study(hidden_size1,hidden_size2,num_layers1,num_layers2,sequence_length,len(vocab),emb_dim,train_batches,dev_batches,learning_rate,momentum,num_epoch,num_features,test_X,test_y)
